defmodule Rumbl.VideoControllerTest do
  ...
  test "shows chosen resource", %{conn: conn} do
    changeset = Rumbl.User.registration_changeset(%Rumbl.User{}, %{name: "Foo", username: "foo", email: "test@test.com", password: "password" })
    user = Repo.insert!(changeset)
    conn = conn()
      |> post("/sessions", %{"session" => %{"username" => "foo", "password" => "password"}})
